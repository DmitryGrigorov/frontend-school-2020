...
/*
    ...
    function delay(time) {
    const deleyPromise = new Promise(function(resolve, reject) {
        setTimeout(() => {
        resolve();
        }, time);
    });
...
/*
    ...
    function responseServer (urls) {
        let array = new Array();
        let i = 0;
        urls.map(key => {
            return fetch(key).then(response => response.json()).then(data => {
                array[i++] = data;
            });
        });
        return array;
    ...
    console.log(responseServer(urls));
