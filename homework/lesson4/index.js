...
function calcObj (name, count, prices){
    ...
    let g;
    for (let name_fruit in prices) {
      if (name === name_fruit) {
       if (prices[name_fruit]===undefined){
         alert("Извините, товар закончился");
       }
       else {count*prices[name_fruit]};
      }
      return;
    }
...
let dog = {
  name: 'Bobik',
  sayvoice: function (countvoice){
    

    let voice = ' bark ';
    for (i=0; i<countvoice; i++)
    {
    voice += 'bark ';
  }
  return alert(this.name+voice);

  }
}
console.log(dog.sayvoice(5));
